...
## 4. Phase 1 — Describe Your Intent

**File name:** `github-copilot-plan-agent-describe-intent-prompt.png`

**Alt text:** Writing a precise prompt for GitHub Copilot's Plan agent — vague vs precise intent description in the Copilot Chat panel

**Description:** Illustrates the first phase of the Plan agent workflow: the developer typing a prompt in the Copilot Chat panel. Contrasts a vague prompt with a precise one to reinforce the key lesson that prompt specificity directly affects plan quality. Used at the start of Phase 1 to set expectations before the workflow details are explained.

**Generation prompt:**
Dark tech illustration. A Copilot Chat panel dominates the center of the frame. Two chat input areas are shown stacked vertically, connected by a "vs" divider. The upper input shows a short, vague message ("Add authentication to the portal API.") with a soft amber warning glow — suggesting ambiguity. The lower input shows a longer, precise multi-line message with structured details (project name, library, constraint) glowing in soft teal — suggesting clarity and control. A GitHub Copilot logo silhouette sits at the top of the panel. Small annotation labels float beside each input: "More questions →" next to the vague prompt, "Faster to plan →" next to the precise one. Background: near-black. Color palette: amber for vague, teal for precise, electric blue for the Copilot logo. Flat vector with chat panel chrome.

---

## 5. Phase 2 — Copilot Scans Your Codebase (Read-Only)
...
## 7. Phase 4 — You Edit the Plan Directly
...
## 9. Writing Better Prompts for the Plan Agent

**File name:** `github-copilot-plan-agent-prompt-patterns-5.png`

**Alt text:** Five prompt patterns for GitHub Copilot's Plan agent — scope by layer, reference existing patterns, specify test coverage, use constraints, describe the end state

**Description:** Illustrates the five prompt-writing patterns covered in the section. Used at the start of the section to give a quick visual summary of all five strategies before each is explained in depth.

**Generation prompt (Version 1 — grid layout):**
Dark tech illustration. Five prompt-pattern cards arranged in a 3+2 grid (three on top row, two centred on bottom row). Each card has a bold number (1–5), a small icon, and a short label. Card 1: a stack of layers icon + "Scope by Layer" (blue). Card 2: a copy/reference icon + "Reference Existing Patterns" (teal). Card 3: a test-tube icon + "Specify Test Coverage" (green). Card 4: a padlock icon + "Use Constraints" (amber). Card 5: a target/bullseye icon + "Describe the End State" (soft purple). Each card is separated by a thin glowing border matching its accent color. A faint chat bubble in the background suggests these are prompt strategies. Background: near-black with a subtle grid. Flat vector card-grid style.

**Generation prompt (Version 2 — vertical list with contrast):**
Dark tech illustration. A vertical list of five prompt-pattern rows, each occupying a horizontal band across the frame. Each row contains: a numbered circle on the left (1–5), a small icon in the center-left, a bold short label, and a faint example snippet on the right rendered in monospaced soft white text. Row 1 (blue): layers icon, "Scope by Layer", snippet "...AccountRepository class only...". Row 2 (teal): reference icon, "Reference Existing Patterns", snippet "...follow the same pattern as OrderService...". Row 3 (green): test-tube icon, "Specify Test Coverage", snippet "...include unit tests using Moq...". Row 4 (amber): padlock icon, "Use Constraints", snippet "...do not modify WebhookController...". Row 5 (purple): bullseye icon, "Describe the End State", snippet "...resilient to transient failures...". Each row glows faintly with its accent color. A vertical teal line connects the numbered circles, suggesting a progression. Background: near-black. Flat vector style.
